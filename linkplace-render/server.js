...
1b) GRAMMATICAL FIT CHECK: When doing a natural-fit replacement, the anchor text must grammatically fit the sentence structure it's replacing. Specifically, check part-of-speech compatibility: if the original phrase was a gerund/noun-phrase (e.g., 'building an AI agent'), and the anchor is an infinitive/imperative phrase (e.g., 'how to build an AI agent'), verify the resulting sentence still reads as correct English. Read the full edited sentence aloud in your reasoning — if it sounds grammatically awkward or broken (e.g., 'the ideal of how to build an AI agent' instead of 'the ideal of building an AI agent'), REJECT this natural-fit approach and either rephrase the surrounding sentence slightly to accommodate the anchor's grammatical form, or switch to a bridging-sentence approach instead.
