...
async function runAnalysis(domain, anchor, linkto, excludedParagraphs = [], excludedArticleUrls = []) {
  const poolKey = `pool::${domain}::${anchor.toLowerCase().trim()}::${linkto.toLowerCase().trim()}::true::${excludedArticleUrls.length}`;
...
router.post("/", async (req, res) => {
  ...
  const cacheKey = `${domain}::${anchorList.join("||").toLowerCase().trim()}::${linkto.toLowerCase().trim()}::${excludeHash}::true`;
